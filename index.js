...
app.post("/api/post", (req, res) => {
  ...
  const sqlInsert =
    "INSERT INTO employee (firstName, lastName, email, phoneNumber, passwords, confirmPasswords, age, gender, department, interests, birthDate) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.status(400).json({ error: "Fail to insert data." });
      }
...
app.get("/api/get", (req, res) => {
  ...
  db.query(sqlGet, (err, result) => {
    if (err) {
      console.log("Error:- ", err);
      return res.status(400).json({ error: "Fail to fetch data." });
    }
    res.send(result);
  });
});
...
app.delete("/api/delete/:id", (req, res) => {
  ...
  db.query(sqlDelete, id, (err, result) => {
    if (err) {
      console.log("Error:- ", err);
      return res.status(400).json({ error: "Fail to delete data." });
    }
...
app.put("/api/update/:id", (req, res) => {
  ...
  const sqlUpdate =
    "UPDATE employee SET firstName=?, lastName=?, email=?, phoneNumber=?, passwords=?, confirmPasswords=?, age=?, gender=?, department=?, interests=?, birthDate=? WHERE id=?";
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        console.log("Error:- ", err);
        return res.status(400).json({ error: "Fail to update data." });
      }
...
app.get("/api/search", (req, res) => {
  ...
  const sqlSearch =
    "SELECT * FROM employee WHERE firstName LIKE ? OR lastName LIKE ? OR email LIKE ? OR phoneNumber LIKE ?";
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        console.log("Error:- ", err);
        return res.status(400).json({ error: "Fail to search data." });
      }
      res.send(result);
    }
  );
...
app.get("/api/sort", (req, res) => {
  ...
  db.query(sqlSort, (err, result) => {
    if (err) {
      console.log("Error:- ", err);
      return res.status(400).json({ error: "Fail to sort" });
    }
    res.send(result);
  });
});
